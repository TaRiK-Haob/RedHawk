...
- recon agents (web-recon, host-recon) use a cheaper/faster model when
  OPENAI_API_MODEL_RECON is set, else fall back to the main model.
- the coordinator and working-tier agents (web-pentest, exploit-dev) use the
  main model (OPENAI_API_MODEL).
...
_BASE_URL = os.getenv("OPENAI_API_BASE_URL", "https://api.deepseek.com")
_API_KEY = os.environ["OPENAI_API_KEY"]
_MAIN_MODEL = os.getenv("OPENAI_API_MODEL", "deepseek-v4-flash")
...
def build_recon_model() -> ChatDeepSeek:
    ...
    Override via OPENAI_API_MODEL_RECON; otherwise falls back to the main model
    so a single-model .env still works.
    """
    return ChatDeepSeek(
        model=os.getenv("OPENAI_API_MODEL_RECON", _MAIN_MODEL),
        base_url=_BASE_URL,
        api_key=_API_KEY,
        temperature=0.1,
    )
